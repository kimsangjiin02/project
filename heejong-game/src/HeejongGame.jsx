...
//   .prologue-text{font-size:18px;line-height:2.2;color:rgba(253,248,240,0.9);text-align:center;max-width:640px;padding:0 28px;flex-shrink:0}
...
//   .name-desc{font-size:17px;color:rgba(253,248,240,0.88);text-align:center;line-height:1.9}
//   .name-avatar-wrap{width:160px;height:160px;border-radius:50%;background:#3a2a1a;border:2px solid #6a5030;display:flex;align-items:center;justify-content:center;overflow:hidden}
...
//       if(data.success) setTimeout(()=>setShowResult(true),800);
...
//       setSuccess(data.success);
//       if(data.success) setTimeout(()=>setShowResult(true),800);
//       else setTimeout(()=>onResult("userDeath"),800);
//     } catch {
//       alert("서버 연결 오류. 다시 시도해주세요.");
//       setRevealed(false); setSelected(null);
//     }
...
// 태황대군: 심기에 따라 이미지 선택
function getTyrantImage(hp) {
  if (hp >= 20) return IMAGES.tyrantHappy;
  if (hp >= 10) return IMAGES.tyrantNormal;
  return IMAGES.tyrantAngry;
}
...
function resolveBossImageUrl(data, fallbackHp = 20) {
  ...
  if (fallbackHp >= 20) return IMAGES.tyrantHappy;
  if (fallbackHp >= 10) return IMAGES.tyrantNormal;
  return IMAGES.tyrantAngry;
}
...
const styles = `
  ...
  .narr-pretendard { font-family: 'Pretendard', sans-serif !important; }
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  *{font-family:'ShillaCulture',serif !important}
  :root{
    --bg:#2b1d0f;
  ...
  /* ── 버튼 ── */
  .btn-prev{background:#5c5248;color:#d8ccc0;border:none;border-radius:6px;padding:13px 28px;font-size:17px;cursor:pointer;letter-spacing:0.03em}
  .btn-next{background:#3a2c1a;color:#e8c97a;border:2px solid #c9a84c;border-radius:6px;padding:13px 32px;font-size:17px;cursor:pointer;letter-spacing:0.03em}
  .btn-next:disabled{opacity:0.4;cursor:default}
  .btn-start{background:#5a4420;color:#e8c97a;border:2px solid #c9a84c;border-radius:6px;padding:13px 0;font-size:17px;cursor:pointer;width:75%;max-width:240px;letter-spacing:0.04em}
  .btn-start:disabled{opacity:0.4;cursor:default}
  ...
  .name-avatar-placeholder{font-size:36px;color:#6a5030}
  .name-row{display:flex;align-items:center;gap:8px;width:100%}
  .name-label{font-size:17px;color:rgba(253,248,240,0.8);white-space:nowrap;flex-shrink:0}
  ...
  .goal-illust-placeholder{width:120px;height:100px}
  .goal-box,.stat-box{background:var(--paper-light);border-radius:6px;padding:22px 24px;width:100%}
  .goal-box p,.stat-box-desc{font-size:17px;color:var(--ink);line-height:2.0;text-align:center}
  .stat-box-desc{margin-bottom:20px}
  .stat-icons-row{display:flex;justify-content:space-around;align-items:flex-end;gap:12px}
  .stat-item-wrap{display:flex;flex-direction:column;align-items:center;gap:10px}
  ...
  .week-card-title{font-size:32px;font-weight:700;color:#1a1208;margin-bottom:18px;letter-spacing:0.06em}
  .week-card-text{font-size:17px;line-height:2.0;color:#1a1208;white-space:pre-line;margin-bottom:24px}
  @media(min-width:768px){
    .week-card{width:58%;max-width:800px;padding:48px 56px}
    .week-card-title{font-size:36px;margin-bottom:28px}
    .week-card-text{font-size:16px;line-height:2.2;margin-bottom:32px}
  ...
  .sbtn:disabled{opacity:0.5}
  .dcnt{font-size:10px;color:#6a5838;text-align:center;padding:2px 0;flex-shrink:0;background:var(--gray-dark)}
  .chat.boss .dcnt{background:#280808;color:#f5c0c0;font-size:13px;padding:4px 0}
...
${currentNarration.tip}`
      ...
      {isBoss && (
        <div className="boss-pcl" style={{display:"none"}}>
          {IMAGES.bossLeftBg && <img src={IMAGES.bossLeftBg} alt="보스패널" className="boss-pcl-bg"/>}
          <div className="boss-pcl-inner" style={{position:"relative",zIndex:1,padding:"20px 16px"}}>
            <div className="boss-pcl-title">태황대군과의<br/>대화</div>
            <div className="boss-pcl-desc">{info.text}</div>
            {info.tip && <div className="boss-pcl-tip">*Tip: {info.tip}</div>}
          </div>
...
function MiniLetter({ onResult, sessionId }) {
  ...
  async function choose(opt){
    ...
    try{
      ...
      setSuccess(data.success);
      if(data.success) setTimeout(()=>setShowResult(true),300);
      else setTimeout(()=>onResult("userDeath"),300);
    } catch {
      alert("서버 연결 오류. 다시 시도해주세요.");
      setRevealed(false); setSelected(null);
    }
...
function getSoldierConfig(week){
  const configs={
    3:{min:100,max:800,baseSafe:600,label:"철저한",hint:"단속이 있으니 신중하게 모으십시오."},
    5:{min:100,max:500,baseSafe:250,label:"매우 철저한",hint:"지금은 단속이 심하니 소규모로만 움직이십시오."},
    8:{min:100,max:700,baseSafe:350,label:"느슨한",hint:"이번엔 넉넉하게 모아도 될 것 같습니다."},
  };
  const base=configs[week]||configs[3];
  return{...base};
}
  ...
  async function submit(){
    ...
    try{
      const r=await fetch(`${API}/minigame`,{method:"POST",
        ...
        body:JSON.stringify({type:roundKey,choice:String(n),session_id:sessionId})});
      const data=await r.json();
      if(data.success){ setResultSuccess(true); setResultCount(n); setTimeout(()=>setShowResult(true),300); }
      else { onResult("soldierCaught"); }
    } catch {
      alert("서버 연결 오류. 다시 시도해주세요.");
    }
  }
...
export default function HeejongGame() {
  ...
  function handleMiniResult(result,value){
    ...
    if(result==="allyOk"||result==="letterOk"){advanceWeek();return;}
    if(result==="soldierOk"){
      updateStats({soldiers:value});
      if(week===8){ setScreen("chat"); return; }  // 8주차면 마지막 대화 한 번 더
      advanceWeek();
      return;
    }
  }
  ...
  return (
    <>
      ...
      <div>
        ...
        {screen==="mini_letter"&&<MiniLetter onResult={handleMiniResult} sessionId={sessionId}/>}
        {screen==="mini_soldier"&&<MiniSoldier week={week} currentSoldiers={stats.soldiers} onResult={handleMiniResult} sessionId={sessionId}/>}
        {screen==="ending_confirm"&&(
          <div style={{width:"100vw",height:"100vh",background:"#1a1208",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24}}>
            <div style={{fontSize:22,color:"#e8c97a",fontWeight:700,letterSpacing:"0.08em"}}>모든 준비가 끝났습니다.</div>
            <div style={{fontSize:15,color:"rgba(253,248,240,0.8)",textAlign:"center",lineHeight:2}}>희종과 함께한 8주간의 여정이 마무리되었습니다.<br/>이제 결말을 확인하시겠습니까?</div>
            <button className="btn-next" style={{fontSize:18,padding:"14px 40px"}} onClick={()=>{
              if(stats.affection<90) triggerEnding("failPersuade");
              else if(stats.soldiers<1000) triggerEnding("deathBattle");
              else triggerEnding("success");
            }}>엔딩 보러가기</button>
