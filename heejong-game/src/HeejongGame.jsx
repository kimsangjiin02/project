...
const styles = `
  @font-face { font-family: 'ShillaCulture'; src: url('/assets/fonts/Shilla_Culture(M).ttf') format('truetype'); font-weight: 400; }
  @font-face { font-family: 'ShillaCulture'; src: url('/assets/fonts/Shilla_Culture(B).ttf') format('truetype'); font-weight: 700; }
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  *{font-family:'ShillaCulture',serif !important}
  :root{--bg:#2b1d0f;--gray:#d9d5ce;--gray-dark:#c8c4bc;--paper:#f5ead6;--paper-light:#fdf8ee;--ink:#2c1a0e;--gold:#c9a84c;--gold2:#e8c97a;--white:#fdf8f0;--safe-top:env(safe-area-inset-top,0px);--safe-bot:env(safe-area-inset-bottom,0px);}
  ...
  .week-card{position:relative;z-index:2;width:58%;max-width:800px;background:rgba(220,225,230,0.82);border-radius:4px;padding:48px 56px;text-align:center;}
  ...
  @media(min-width:768px){
    .chat:not(.boss){display:grid!important;grid-template-columns:220px 1fr 270px;grid-template-rows:100vh;background:var(--gray);}
    ...
    .pcl{display:flex!important;flex-direction:column;align-items:center;padding:16px 12px;gap:10px;overflow-y:auto;height:100vh;background:var(--gray);border-right:1px solid #b8b4ac;}
    .pcwk{font-size:30px;font-weight:700;color:var(--ink);background:rgba(245,234,214,0.9);border:1px solid #c0a870;border-radius:4px;padding:4px 12px;width:100%;text-align:left;}
    .pccharimg{width:100%;height:280px;border:1px solid #b8a070;border-radius:4px;overflow:hidden;background:rgba(245,234,214,0.8);display:flex;align-items:center;justify-content:center;flex-direction:column;gap:6px;color:#8a7050;font-size:11px;text-align:center;flex-shrink:0;}
    ...
    .pcr{display:flex!important;flex-direction:column;padding:0;gap:0;overflow-y:auto;height:100vh;background:var(--gray);border-left:1px solid #b8b4ac;position:relative;}
    .pcr-scroll-img{width:100%;flex-shrink:0}
    .pcr-scroll-img img{width:100%;height:auto;object-fit:contain;display:block}
    .pcr-scroll-fallback{background:rgba(245,234,214,0.9);border:1px solid #c0a060;margin:12px;border-radius:4px;padding:14px 12px;font-size:12px;line-height:1.85;color:var(--ink);}
    ...
    .pcrec-btn{background:#fff;border:1px solid #d0c8b8;border-radius:8px;padding:10px 14px;font-size:13px;color:#3a2a1a;cursor:pointer;text-align:left;line-height:1.4;transition:background 0.15s;box-shadow:0 1px 2px rgba(0,0,0,0.06);}
...
function BottomNav({ onPrev, onNext, nextLabel="다음", nextDisabled=false }) {
  return (
    <div className="bottom-nav">
      <button className="btn-prev" onClick={onPrev} style={{visibility:onPrev?"visible":"hidden"}}>이전</button>
  ...
  return (
    <div className="page fade" key={page}>
      <div className="prologue-wrap">
        {IMAGES.prologue ? <img src={IMAGES.prologue} className="prologue-illust" alt="프롤로그"/> : <div className="prologue-illust-placeholder">🏔️</div>}
        <p className="prologue-text">{PROLOGUE_PAGES[page]}</p>
        <div className="prologue-dots">{PROLOGUE_PAGES.map((_,i)=><div key={i} className={`dot${i===page?" a":""}`}/>)}</div>
      </div>
      <div className="bottom-nav">
        <button className="btn-prev" onClick={()=>setPage(p=>p-1)} style={{visibility:page>0?"visible":"hidden"}}>이전</button>
        {page<PROLOGUE_PAGES.length-1 ? <button className="btn-next" onClick={()=>setPage(p=>p+1)}>다음</button> : <button className="btn-next" onClick={onNext}>다음</button>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="page fade">
      <div className="name-wrap">
        <p className="name-desc">당신은 그런 희종을 어린 시절부터 지켜본<br/>희종의 또 다른 숙부 OO대군이다.</p>
        <div className="name-avatar-wrap">
          {IMAGES.playerAvatar ? <img src={IMAGES.playerAvatar} alt="플레이어"/> : <div className="name-avatar-placeholder">?</div>}
        </div>
        <div className="name-row">
          <span className="name-label">이름:</span>
          <input className="name-input" placeholder="(직접 입력) 대군" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&name.trim()&&onNext(name.trim())}/>
        </div>
        <button className="btn-start" onClick={()=>name.trim()&&onNext(name.trim())} disabled={!name.trim()}>채팅 시작하기</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="page fade">
      <div className="goal-wrap">
        ...
        {IMAGES.phoenix ? <img src={IMAGES.phoenix} className="goal-illust" alt="봉황"/> : <div className="goal-illust-placeholder">🦅</div>}
        <div className="goal-box"><p>8주 동안 좌절에 빠진 희종을 설득하여<br/>태황 대군의 폭정 아래에서 위기에 빠진 조선을 구하라.</p></div>
      </div>
          ...
          <div className="stat-icons-row">
            {stats.map(s=>(
              <div className="stat-item-wrap" key={s.label}>
                {IMAGES[s.key] ? <img src={IMAGES[s.key]} className="stat-item-img" alt={s.label}/> : <div className="stat-item-placeholder">{s.fallback}</div>}
...
function WeekTransitionScreen({ week, onNext }) {
  ...
  return (
    <div className="week-screen">
      {IMAGES.weekBg ? <img src={IMAGES.weekBg} alt="배경" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",zIndex:0}}/> : <div style={{position:"absolute",inset:0,background:"#2b1d0f",zIndex:0}}/>}
...
function ChatScreen({ week, playerName, stats, onStatsChange, onDialogComplete, isBoss=false, badFaction="죽", sessionId }) {
  ...
  const [affDelta, setAffDelta] = useState(null);
  const [recAnswers, setRecAnswers] = useState(INIT_RECS[week]||[]);
  const [narrOpen, setNarrOpen] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [currentCharImg, setCurrentCharImg] = useState(null);
  const endRef = useRef(null);

  useEffect(()=>{ endRef.current?.scrollIntoView({behavior:"smooth"}); },[messages]);

  useEffect(()=>{
    if(isBoss) fetch(`${API}/tyrant/start`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:sessionId})}).catch(()=>{});
    return ()=>{ if(isBoss) fetch(`${API}/tyrant/end`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:sessionId})}).catch(()=>{}); };
  },[isBoss, sessionId]);

  // 백엔드 URL 우선, 없으면 호감도 기반 로컬 이미지

   const charImg = isBoss
    ? (IMAGES.tyrant || null)
    : (currentCharImg || getHeejongImage(stats.affection));
  ...
  async function handleBackendResponse(data, nc) {
    ...
    if (대사.includes('{"') || 대사.includes('"호감도변화"')) {
      try {
        const p = JSON.parse(대사);
        대사 = p.대사 || 대사;
      } catch {
        const bi = 대사.indexOf("{");
        if (bi > 0) 대사 = 대사.slice(0, bi).trim();
      }
    }
    ...
    // 이미지 업데이트
    if (!isBoss) {
      const imgUrl = data.이미지URL || null;
      if (imgUrl) {
        setCurrentCharImg(imgUrl);
      }
    } else {
    ...
    if (isBoss) {
      ...
      if (newHp <= 0) {
        setTimeout(() => onDialogComplete("bossKill"), 1500);
        return;
      }

      if (nc >= maxDialog) {
        setTimeout(() => onDialogComplete("bossSurvive"), 1500);
        return;
      }
    } else {
      const ad = data.stats?.호감도변화 || 0;

      setAffDelta(ad);
      onStatsChange({ affection: ad });

      const newAff = stats.affection + ad;

      if (newAff <= -20) {
        setTimeout(() => onDialogComplete("badAffection"), 1500);
        return;
      }
  ...
  async function sendRecommended(text) {
    ...
    try {
      ...
      if (nc >= maxDialog) {
        setTimeout(() => onDialogComplete(isBoss ? "bossSurvive" : "complete"), 1500);
      }
    }
  }
    ...
    <div className={`chat${isBoss?" boss":""}`}>
      <div className="mob-topbar">
        ...
        <div className="schips">
          {isBoss ? <div className="schip">심기 <b style={{color:"#ff6b6b"}}>{bossHp}</b></div>
            : <><div className="schip">❤️<b>{stats.affection}</b></div><div className="schip">⚔️<b>{stats.soldiers}</b></div><div className="schip">🏮<b>{stats.minsim}</b></div></>}
        </div>
        <div className="cthumb" onClick={()=>setShowModal(true)}>{charImg?<img src={charImg} alt={npcName}/>:(isBoss?"👑":"🧑")}</div>
      </div>
      {isBoss&&(<div className="mob-bossbar"><span className="bblbl">심기</span><div className="bbtrk"><div className="bbfil" style={{width:`${hpPct}%`}}/></div><span className="bbval">{bossHp}/30{bossDelta!==null&&<span style={{fontSize:10,color:bossDelta>=0?"#69f0ae":"#ff5252",marginLeft:4}}>{bossDelta>=0?`+${bossDelta}`:bossDelta}</span>}</span></div>)}
      ...
      {!isBoss&&(
        <div className="pcl" style={{display:"none"}}>
          ...
          <div className="pccharimg">{charImg?<img src={charImg} alt="희종"/>:<><span style={{fontSize:36}}>🧑</span><span>(호감도별 변화 이미지)</span></>}</div>
          <div className="pccharimg-label">(호감도 별 변화 이미지)</div>
          <div className="pcsts">
            <div className="pcst"><div className="pcstn">호감도</div><div className="pcstv">{stats.affection}</div><div style={{fontSize:10,fontWeight:700,minHeight:14,color:affDelta>0?"#2e7d32":affDelta<0?"#c62828":"transparent"}}>{affDelta!==null&&affDelta!==0?(affDelta>0?`+${affDelta}`:affDelta):""}</div></div>
            <div className="pcst"><div className="pcstn">민심</div><div className="pcstv">{stats.minsim}</div></div>
            <div className="pcst"><div className="pcstn">사병</div><div className="pcstv">{stats.soldiers}</div><div className="pcst-sub">/1000</div></div>
          </div>
      ...
      <div className="pcc" style={{display:"contents"}}>
        <div className="pcchdr" style={{display:"none"}}>{isBoss?"⚠️ 태황대군과의 대화 — 심기를 건드리지 마세요!":`*${maxDialog}번의 대화가 진행되면 다음 주차로 넘어갑니다.`}</div>
        <div className="msgs">
          {messages.map((m,i)=>(
      ...
      {!isBoss&&(
        <div className="recbar">
          <div className="recscr">
            {recAnswers.map((a,i)=><button key={i} className="rchip" onClick={()=>sendRecommended(a)} disabled={loading}>{a}</button>)}
          </div>
        </div>
      )}
      ...
      {!isBoss&&(
        <div className="pcr" style={{display:"none"}}>
          {IMAGES.scrollBg ? <div className="pcr-scroll-img"><img src={IMAGES.scrollBg} alt="두루마리"/></div>
            : <div className="pcr-scroll-fallback"><div className="scroll-title">📜 나레이션</div><p style={{fontSize:12,lineHeight:1.9,whiteSpace:"pre-line"}}>{info.text}</p>{info.tip&&<p className="tip-text">*Tip: {info.tip}</p>}</div>}
          <div className="pcr-content">
            <div className="pcrec-wrap">
              {recAnswers.map((a,i)=><button key={i} className="pcrec-btn" onClick={()=>sendRecommended(a)} disabled={loading}>{a}</button>)}
            </div>
          </div>
        </div>
      )}
      {isBoss&&(
        <div className="boss-pcr" style={{display:"none"}}>
          <div className="boss-pcr-charimg">{IMAGES.tyrant?<img src={IMAGES.tyrant} alt="태황대군" style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"top"}}/>:<span style={{fontSize:14}}>태황대군 이미지</span>}</div>
          <div className="boss-pcr-hp">
            <div className="boss-pcr-hplbl">심기</div>
            <div className="boss-pcr-hpval">{bossHp}</div>
            {bossDelta!==null&&<div className="boss-pcr-delta" style={{color:bossDelta>=0?"#69f0ae":"#ff5252"}}>{bossDelta>=0?`+${bossDelta}`:bossDelta}</div>}
      ...
      {showModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowModal(false)}>
          <div style={{background:"#f5ead6",borderRadius:12,padding:20,maxWidth:260,width:"90%",textAlign:"center"}} onClick={e=>e.stopPropagation()}>
            <div style={{width:150,height:190,background:"#d4c4a0",borderRadius:8,margin:"0 auto 12px",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#8a7050"}}>
              {charImg?<img src={charImg} alt={npcName} style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"top"}}/>:"캐릭터 이미지"}
            </div>
...
function MiniAlly({ onResult, badFaction="죽", sessionId }) {
  ...
  async function choose(opt) {
    ...
    try {
      const r=await fetch(`${API}/minigame`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"협력세력_1차",choice:opt,session_id:sessionId})});
      const data=await r.json();
      setSuccess(data.success);
      setTimeout(()=>onResult(data.success?"allyOk":"evil"),1400);
    } catch { setTimeout(()=>onResult(opt===badFaction?"evil":"allyOk"),1400); }
  }
  ...
  return (
    <div className="mini">
      <div className="mboard" style={IMAGES.miniBg?{backgroundImage:`url(${IMAGES.miniBg})`,backgroundSize:"100% 100%",border:"none",borderRadius:6}:{}}>
        ...
        <div className="mopts">
          {OPTIONS.map(opt=>(
            <button key={opt} className={`mbtn${revealed&&opt===selected?(success===null?"":(success?" sel":" wrong")):""}` } onClick={()=>!revealed&&choose(opt)} disabled={revealed}>'{opt}' 세력</button>
          ))}
        </div>
        {revealed&&success!==null&&<p style={{marginTop:14,fontSize:13,fontWeight:700,color:success?"#2e7d32":"#c62828"}}>{success?"✅ 현명한 선택입니다!":"❌ 반(反)희종 세력이었습니다!"}</p>}
      </div>
    </div>
  );
}
...
function MiniLetter({ onResult, sessionId }) {
  ...
  async function choose(opt) {
    setSelected(opt); setRevealed(true);
    try {
      const r=await fetch(`${API}/minigame`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"비밀서신",choice:opt,session_id:sessionId})});
      const data=await r.json();
      setSuccess(data.success);
  ...
  return (
    <div className="mini">
      <div className="mboard" style={IMAGES.miniBg?{backgroundImage:`url(${IMAGES.miniBg})`,backgroundSize:"100% 100%",border:"none",borderRadius:6}:{}}>
        ...
        <div className="mopts">
          {OPTIONS.map(opt=>(
            <button key={opt} className={`mbtn${revealed&&opt===selected?(success===null?"":(success?" sel":" wrong")):""}`} onClick={()=>!revealed&&choose(opt)} disabled={revealed}>{opt}</button>
          ))}
        </div>
        {revealed&&success!==null&&<p style={{marginTop:14,fontSize:13,fontWeight:700,color:success?"#2e7d32":"#c62828"}}>{success?"✅ 서신을 안전하게 전달했습니다!":"❌ 심복이 매복한 길이었습니다!"}</p>}
      </div>
    </div>
  );
}
...
function MiniSoldier({ week, currentSoldiers, onResult, sessionId }) {
  ...
  async function submit() {
    ...
    try {
      const r=await fetch(`${API}/minigame`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:roundKey,choice:String(n),session_id:sessionId})});
      const data=await r.json();
      if(data.success) onResult("soldierOk",n); else onResult("soldierCaught");
    } catch { if(n>cfg.safeMax) onResult("soldierCaught"); else onResult("soldierOk",n); }
  }
    ...
    <div className="mini">
      <div className="mboard" style={IMAGES.miniBgSoldier?{backgroundImage:`url(${IMAGES.miniBgSoldier})`,backgroundSize:"100% 100%",border:"none",borderRadius:6}:IMAGES.miniBg?{backgroundImage:`url(${IMAGES.miniBg})`,backgroundSize:"100% 100%",border:"none",borderRadius:6}:{}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",alignItems:"center",marginTop:8}}>
          <input className="minp" type="number" min={cfg.min} max={cfg.max} placeholder={`${cfg.min}~${cfg.max}`} value={input} style={{width:"70%",marginBottom:0}} onChange={e=>{setInput(e.target.value);setError("");}} onKeyDown={e=>e.key==="Enter"&&submit()}/>
...
function EndingScreen({ endingKey, onRestart }) {
  ...
  return (
    <div className="ending fade">
      ...
      <div className="eimg-wrap">{endImg?<img src={endImg} alt="엔딩"/>:<span style={{color:"rgba(201,168,76,0.4)",fontSize:11}}>(엔딩 이미지)</span>}</div>
      ...
      <button className="btn-next" style={{maxWidth:280,width:"100%"}} onClick={onRestart}>처음부터 다시하기</button>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div>
        ...
        {screen==="chat"&&<ChatScreen week={week} playerName={playerName} stats={stats} onStatsChange={updateStats} onDialogComplete={handleDialogComplete} isBoss={week===5} badFaction={badFaction} sessionId={sessionId}/>}
